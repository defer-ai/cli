...
func fakeDecisions() []decision.Decision {
	return []decision.Decision{
		{
			ID: "STACK-001", Category: "Stack", Question: "Backend language?",
		...
		{
			ID: "STACK-002", Category: "Stack", Question: "Frontend framework?",
			Options: []decision.DecisionOption{
				{Key: "A", Label: "React"},
				{Key: "B", Label: "Vue"},
				{Key: "C", Label: "Choose for me"},
		...
		{
			ID: "UI-001", Category: "UI", Question: "CSS approach?",
			Options: []decision.DecisionOption{
...
func TestReviseDecision(t *testing.T) {
	...
	// Revise STACK-001
	m, _ = updateModel(t, m, ReviseDecisionMsg{ID: "STACK-001", NewAnswer: "TypeScript"})
	...
	if !found {
		t.Fatal("STACK-001 not found in tree decisions")
	}
}
...
func TestReviseTriggersExecutorsWhenAllAnswered(t *testing.T) {
	...
	decs := []decision.Decision{
		{ID: "STACK-001", Category: "Stack", Question: "Language?",
			...
			Source:  "user"},
		{ID: "UI-001", Category: "UI", Question: "Framework?",
			Options: []decision.DecisionOption{{Key: "A", Label: "React"}},
			Source:  "user"},
		{ID: "UI-002", Category: "UI", Question: "State management?",
	...
	// Since autoIDs has STACK-001, the Agent.AutoDecide call only auto-decides
	...
	var cmd tea.Cmd
	m, cmd = updateModel(t, m, ReviseDecisionMsg{ID: "UI-001", NewAnswer: "React"})
	...
	// Still one more pending (UI-002)
	if m.executorsLaunched {
		t.Fatal("executors should not be launched yet (UI-002 still pending)")
	}

	// Answer the last pending decision
	m, cmd = updateModel(t, m, ReviseDecisionMsg{ID: "UI-002", NewAnswer: "Redux"})
	msg = processCmd(t, cmd)
	if _, ok := msg.(CheckAllDecidedMsg); !ok {
		t.Fatalf("expected CheckAllDecidedMsg, got %T", msg)
	}
...
func TestReviseAfterExecutionRelaunches(t *testing.T) {
	...
	m, _ = updateModel(t, m, ReviseDecisionMsg{ID: "STACK-001", NewAnswer: "Rust"})
...
func TestExecutorDecisionsMerge(t *testing.T) {
	...
	newDec := decision.Decision{
		ID: "DATA-001", Category: "Data", Question: "Database choice?",
	...
	if countWithID > 1 {
		t.Errorf("DATA-001 appears %d times, should be 1 (no duplication)", countWithID)
	}
...
func TestSuggestReplacesOptions(t *testing.T) {
	...
	m, _ = updateModel(t, m, SuggestResponseMsg{ID: "STACK-001", Options: newOpts})
