...
func managerDecisions() []decision.Decision {
	return []decision.Decision{
		{ID: "STACK-001", Category: "Stack", Question: "Language?",
			...
			Source: "user"},
		{ID: "STACK-002", Category: "Stack", Question: "Framework?",
		...
		{ID: "UI-001", Category: "UI", Question: "CSS approach?",
			...
			Source: "user"},
		{ID: "DATA-001", Category: "Data", Question: "Database?",
...
func TestAutoDecidePicksFirstNonChooseForMe(t *testing.T) {
	...
	for _, d := range decs {
		if d.ID == "STACK-001" {
			...
			if *d.Answer != "Go" {
				t.Errorf("STACK-001 answer = %q, want Go (first non-choose)", *d.Answer)
			}
		}
	}
}
...
func TestSyncDecisionsAddsNew(t *testing.T) {
	...
	if !found {
		t.Error("NEW-001 not added to allDecs")
	}
}
