...
const PHASE_ORDER: Phase[] = [
  ...
  "domains", "tree", "pick-1", "pick-2", "executing", "mid-pause",
  "mid-pick", "resuming", "done",
];
...
function phaseToMood(p: Phase) {
  if (p === "idle") return "idle";
  if (p === "done") return "done";
  if (p === "thinking-1" || p === "thinking-2" || p === "thinking-3" || p === "executing" || p === "resuming") return "thinking";
  return "asking";
}
...
export function Demo() {
  ...
  }, [phase, typed, toolIdx, toolIdx2, pick1, pick2, midPick]);
  ...
  const reset = () => {
    ...
    setMidPick(null); setToolIdx(0); setToolIdx2(0); setDomains(INIT_DOMAINS);
  };
    ...
    <div className="border border-border rounded-xl bg-surface overflow-hidden">
      ...
      <div ref={containerRef} className="font-mono text-xs max-h-[600px] overflow-y-auto">
        <div className="flex p-4 gap-6">
          <div className="shrink-0 hidden sm:flex items-start pt-2">
            <WebMascot mood={phaseToMood(phase)} pixelSize={4} />
          </div>
          ...
          <div className="flex-1 space-y-3">
            ...
            {ord(phase, "tree") && (
              <div className="space-y-1 border-t border-border/30 pt-3">
                ...
                {domains.filter((d) => d.mode === "auto").map((d) => (
                  <div key={d.name} className="space-y-0.5">
                    <div className="text-gray-500 text-[10px]">{d.name}</div>
                    {(d.name === "Stack"
                      ...
                      <div key={q} className="text-gray-500 pl-2">
                        <span>{"▪ "}</span>{q} <span className="text-gray-600">{a}</span>
                      </div>
                    ))}
                  </div>
                ))}
                {domains.filter((d) => d.mode === "review").map((d) => (
                  <div key={d.name} className="space-y-0.5">
                    <div className="text-gray-500 text-[10px]">{d.name}</div>
                    <div className="pl-2">
                      <span className="text-yellow-400">{"○ "}</span>
                      <span className={pick1 !== null ? "text-green-400" : "text-yellow-400"}>
                        Authentication method?{pick1 !== null && <span className="text-gray-600"> {["JWT tokens", "Session-based", "OAuth2"][pick1]}</span>}
                      </span>
                    </div>
                    <div className="pl-2">
                      <span className="text-yellow-400">{"○ "}</span>
                      <span className={pick2 !== null ? "text-green-400" : "text-yellow-400"}>
                        Password storage?{pick2 !== null && <span className="text-gray-600"> {["bcrypt", "argon2"][pick2]}</span>}
                      </span>
                    </div>
                  </div>
                ))}
                {phase === "tree" && reviewCount > 0 && <>
                ...
                {phase === "tree" && reviewCount === 0 && <button onClick={() => setPhase("executing")} className={BTN + " mt-2"}>execute</button>}
              </div>
            )}
            ...
            {ord(phase, "pick-1") && pick1 !== null && pick2 === null && phase !== "executing" && (
              <div className="space-y-2 border-t border-border/30 pt-3">
                <div className="text-green-400 text-[10px] mb-1">{"✓ "}Authentication method: {["JWT tokens", "Session-based", "OAuth2"][pick1]}</div>
                ...
                <OptionList options={["bcrypt", "argon2", "Choose for me"]} delegate="Choose for me" onPick={(i) => { setPick2(i === 2 ? 1 : i); setPhase("executing"); }} />
              </div>
            )}

            {/* Execution */}
            {ord(phase, "executing") && pick2 !== null && (
              <div className="space-y-1 border-t border-border/30 pt-3">
            ...
            {phase === "mid-pause" && (
              <div className="space-y-2 mt-1">
                ...
                <OptionList options={["24 hours", "7 days", "30 days", "Choose for me"]} delegate="Choose for me" onPick={(i) => { setMidPick(i === 3 ? 1 : i); setPhase("resuming"); }} />
              </div>
            )}
            ...
            {ord(phase, "resuming") && midPick !== null && (
              <div className="space-y-1 mt-1">
                <div className="text-green-400 text-[10px]">{"✓ "}Session duration: {["24 hours", "7 days", "30 days"][midPick]}</div>
            ...
            {phase === "done" && (
              <div className="space-y-2 border-t border-border/30 pt-3">
                <div className="text-green-400 font-bold">{"✓ "}Done. 9 decisions tracked.</div>
                <div className="text-gray-500 text-[10px]">
                  {"  "}<span className="text-gray-400">7</span> auto-decided{"  "}<span className="text-green-400">3</span> you confirmed{"  "}<span className="text-yellow-400">2</span> discovered during execution
                </div>
      ...
      <div className="px-4 py-2 border-t border-border/50 flex justify-between">
        <span className="text-gray-600 text-[10px] font-mono">
          {phase === "idle" ? "ready" : phase === "typing" || phase === "enter" ? "input" : phase.startsWith("thinking") ? "thinking" : phase === "executing" || phase === "resuming" ? "executing" : phase === "done" ? "done | $0.04" : "asking"}
        </span>
