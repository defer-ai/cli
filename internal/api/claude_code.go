...
type ClaudeCodeProvider struct {
	...
	// StrictMode enables the decision-gated executor: native Write/Edit
	// tools are removed, the defer MCP server is loaded via --mcp-config
	// (exposing register_decision + write_file as the only write path),
	// and a system-prompt appendix tells the model to use the gated
	// protocol. Enabled by default for the executor phase.
	StrictMode bool
}
...
func (p *ClaudeCodeProvider) RunCompletion(ctx context.Context, systemPrompt, userPrompt string, events chan<- Event) {
	...
	// Settings file: env var override only (for bench experiments).
	...
	// Tool allowlist via --tools. Priority:
	//   1. DEFER_CLAUDE_TOOLS env var (bench override)
	//   2. p.AllowedTools (decompose/chat phases)
	//   3. strictAllowedTools (executor StrictMode default)
	//   4. Omit the flag (Claude Code default: all tools)
	//
	// p.AllowedTools == ["none"] → --tools "" (disable every tool).
	...
	// gets the full strict appendix and restricted phases get the
	// headless appendix (prevents plugin-skill SessionStart hijacking).
	...
	// p.AllowedTools flows through --tools above.
